...
RSpec.describe ConversationReplyMailer do
  describe 'reply' do
      ...
      context 'when message is a CSAT survey' do
        ...
        it 'includes CSAT survey URL in outgoing_content' do
          with_modified_env 'FRONTEND_URL' => 'https://one-link.kz' do
            ...
            expect(mail.decoded).to include "https://one-link.kz/survey/responses/#{conversation.uuid}"
          end
        end
    ...
    context 'when inbox email address is available' do
      let(:inbox) { create(:inbox, account: account, email_address: 'noreply@one-link.kz') }
