...
RSpec.describe Whatsapp::WebhookRouteRegistryClient do
  ...
  it 'registers and unregisters with method-bound signatures' do
    register_request = stub_request(:put, url).to_return(status: 204)
    delete_request = stub_request(:delete, url).to_return(status: 204)

    expect(client.register!(waba_id: '123', phone_number_id: '456')).to be(true)
    expect(client.unregister!(waba_id: '123', phone_number_id: '456')).to be(true)
