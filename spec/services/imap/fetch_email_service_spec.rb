...
RSpec.describe Imap::FetchEmailService do
  ...
  let(:chatwoot_notification_header) do
    sender = ENV.fetch('MAILER_SENDER_EMAIL', 'Onelink <accounts@one-link.kz>')
    eml_content_with_message_id.sub(/^From:.*\n/, "From: #{sender}\n")
  end
  ...
  describe '#perform' do
    ...
    context 'when new emails are available in the mailbox' do
      ...
      it 'skips Chatwoot notification emails before fetching mail bodies' do
        travel_to '26.10.2020 10:00'.to_datetime do
          notification_header = Net::IMAP::FetchData.new(1, 'BODY[HEADER]' => chatwoot_notification_header)

          allow(imap).to receive(:search).with(%w[SINCE 25-Oct-2020]).and_return([1])
          allow(imap).to receive(:fetch).with([1], 'BODY.PEEK[HEADER]').and_return([notification_header])
          allow(imap).to receive(:logout)
      ...
      it 'does not count emails without message ids toward the sync limit' do
        travel_to '26.10.2020 10:00'.to_datetime do
          ...
          empty_message_id_seq_nums.each_slice(Imap::BaseFetchEmailService::HEADER_FETCH_BATCH_SIZE) do |batch|
            headers = empty_message_id_headers.select { |header| batch.include?(header.seqno) }
            allow(imap).to receive(:fetch).with(batch, 'BODY.PEEK[HEADER]').and_return(headers)
          end
      ...
      it 'caps body fetches to the maximum messages per sync' do
        travel_to '26.10.2020 10:00'.to_datetime do
          max_messages_per_sync = Imap::BaseFetchEmailService::MAX_MESSAGES_PER_SYNC
          seq_nums = (1..(max_messages_per_sync + 1)).to_a

          seq_nums.each_slice(Imap::BaseFetchEmailService::HEADER_FETCH_BATCH_SIZE) do |batch|
            headers = batch.map do |seq_num|
              header = eml_content_with_message_id.sub(/^Message-ID:.*\n/, "Message-ID: <message-#{seq_num}@example.com>\n")
              Net::IMAP::FetchData.new(seq_num, 'BODY[HEADER]' => header)
            end
            allow(imap).to receive(:fetch).with(batch, 'BODY.PEEK[HEADER]').and_return(headers)
          end

          (1..max_messages_per_sync).each do |seq_num|
            raw_mail = eml_content_with_message_id.sub(/^Message-ID:.*\n/, "Message-ID: <message-#{seq_num}@example.com>\n")
            allow(imap).to receive(:fetch).with(seq_num, 'RFC822').and_return([Net::IMAP::FetchData.new(seq_num, 'RFC822' => raw_mail)])
          end
          allow(imap).to receive(:search).with(%w[SINCE 25-Oct-2020]).and_return(seq_nums)
          allow(imap).to receive(:logout)

          result = described_class.new(channel: imap_email_channel).perform

          expect(result.length).to eq max_messages_per_sync
          expect(imap).not_to have_received(:fetch).with(max_messages_per_sync + 1, 'RFC822')
        end
      end
    end
