...
describe Contacts::ContactableInboxesService do
  ...
  describe '#get' do
    ...
    context 'when the contact phone is stored by the voice call runtime' do
      let(:voice_channel) { create(:channel_voice, :sipuni, account: account, phone_number: '+77000000001') }
      ...
      let(:voice_contact) { create(:contact, account: account, phone_number: nil) }
      let(:voice_conversation) { create(:conversation, account: account, inbox: voice_inbox, contact: voice_contact) }
      let!(:voice_call_session) do
        create(
