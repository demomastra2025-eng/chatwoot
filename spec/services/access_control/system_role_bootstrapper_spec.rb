...
RSpec.describe AccessControl::SystemRoleBootstrapper do
  ...
  describe '.call' do
    it 'creates all system roles and their canonical grants' do
      ...
      expect(AccessRoleGrant.where(account: account, resource: 'automation_rules')).not_to exist

      observer = result.roles_by_key.fetch('observer')
      expected_observer_grants = AccessControl::SystemRoleCatalog::BOOTSTRAP_RESOURCES.map do |resource|
        [resource, 'view', 'all']
      end
    ...
    it 'creates the canonical Department Lead matrix' do
      ...
      expected_lead_grants = AccessControl::SystemRoleCatalog::BOOTSTRAP_RESOURCES.flat_map do |resource|
    ...
    it 'creates the canonical Employee matrix' do
      ...
      expected_employee_grants = AccessControl::SystemRoleCatalog::BOOTSTRAP_RESOURCES.flat_map do |resource|
        capabilities = AccessRoleGrant::RESOURCE_CAPABILITIES.fetch(resource)
    ...
    it 'creates the canonical Commercial Director matrix' do
      ...
      expected_commercial_grants = AccessControl::SystemRoleCatalog::BOOTSTRAP_RESOURCES.flat_map do |resource|
