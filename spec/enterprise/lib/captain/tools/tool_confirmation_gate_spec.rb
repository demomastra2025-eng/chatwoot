...
RSpec.describe Captain::Tools::ToolConfirmationGate do
  ...
  it 'blocks confirmation-required assistant tools until the operator confirms the same arguments', :aggregate_failures do
    service = Captain::Tools::Account::SendMessageToConversationService.new(
      assistant,
      user: user,
  ...
  it 'redacts sensitive values inside JSON string arguments in confirmation previews' do
    service = Captain::Tools::Account::CreateAutomationRuleService.new(
      assistant,
      user: user,
      copilot_thread: copilot_thread
    )
  ...
  it 'redacts secret-like instruction values from confirmation previews' do
    service = Captain::Tools::Account::CreateCaptainScenarioService.new(
      assistant,
      user: user,
      copilot_thread: copilot_thread
    )
  ...
  it 'blocks high-risk built-in assistant tools without an explicit registry confirmation flag' do
    ...
    service = Captain::Tools::Account::CreateDealService.new(
      assistant,
      user: user,
      conversation: conversation,
      copilot_thread: nil
  ...
  it 'does not gate non-confirmation tools' do
    service = Captain::Tools::Account::SearchContactsService.new(
      assistant,
      user: user,
      copilot_thread: copilot_thread
    )
