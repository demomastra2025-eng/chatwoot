...
RSpec.describe Captain::PromptRenderer do
  ...
  describe '.render' do
    ...
    it 'renders snippet partials from the captain prompt snippets directory' do
      allow(File).to receive(:read).with(template_path).and_return("Hello {% render 'user', name: name %}")
      allow(File).to receive(:exist?).with(snippet_template_path.to_s).and_return(true)
      allow(File).to receive(:read).with(snippet_template_path.to_s).and_return('Name: {{ name }}')
  ...
  describe 'snippet rendering' do
    ...
    it 'raises a liquid file system error for missing snippets' do
      allow(File).to receive(:read).with(template_path).and_return("{% render 'nonexistent' %}")

      expect { described_class.render(template_name, {}) }
        .to raise_error(Liquid::FileSystemError, /No such template 'nonexistent'/)
    end
  end
