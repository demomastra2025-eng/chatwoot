...
RSpec.describe 'Captain touch management copilot services' do
  ...
  it 'cancels and deletes single touches through copilot services' do
    ...
    cancel_service = copilot_service(Captain::Tools::Account::CancelTouchService)
    ...
    delete_service = copilot_service(Captain::Tools::Account::DeleteTouchService)
  ...
  it 'creates, applies, cancels, and archives touch plans through copilot services' do
    create_service = copilot_service(Captain::Tools::Account::CreateTouchPlanService)
    ...
    apply_service = copilot_service(Captain::Tools::Account::ApplyTouchPlanService)
    apply_payload = JSON.parse(execute_confirmed(apply_service, touch_plan_id: touch_plan_id))
    created_touch_id = Reminder.last.id
    cancel_service = copilot_service(Captain::Tools::Account::CancelTouchesService)
    cancel_payload = JSON.parse(execute_confirmed(
                                  cancel_service,
                                  touch_plan_id: touch_plan_id,
                                  reason: 'Stop plan'
                                ))
    archive_service = copilot_service(Captain::Tools::Account::ArchiveTouchPlanService)
  ...
  it 'prefers a touch plan name over a conflicting id' do
    ...
    payload = JSON.parse(execute_confirmed(
                           copilot_service(Captain::Tools::Account::ApplyTouchPlanService),
