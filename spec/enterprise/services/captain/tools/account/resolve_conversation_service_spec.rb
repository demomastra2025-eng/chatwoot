...
RSpec.describe Captain::Tools::Account::ResolveConversationService do
  ...
  it 'returns the configured assistant outcome written by the transition' do
    assistant.update!(
      config: {
    ...
    payload = JSON.parse(service.execute(reason: 'Customer confirmed resolution', status_reason: 'customer_confirmed'))
  ...
  it 'rejects a transition without a specific reason' do
    expect(service.execute).to eq('ERROR: ArgumentError: A specific completion explanation is required')
