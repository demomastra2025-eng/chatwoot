...
RSpec.describe 'Captain custom tool admin copilot tools' do
  ...
  describe Captain::Tools::Account::ListCaptainCustomToolsService do
  ...
  describe Captain::Tools::Account::GetCaptainCustomToolService do
    let(:service) { described_class.new(assistant, user: admin) }
  ...
  describe Captain::Tools::Account::CreateCaptainCustomToolService do
    let(:service) { described_class.new(assistant, user: admin) }
  ...
  describe 'write custom tool tools' do
    it 'updates, toggles, and deletes custom tools with redacted rollback payloads', :aggregate_failures do
      update_result = Captain::Tools::Account::UpdateCaptainCustomToolService.new(assistant, user: admin).execute(
      ...
      status_payload = JSON.parse(
        Captain::Tools::Account::SetCaptainCustomToolStatusService.new(assistant, user: admin).execute(tool_id: custom_tool.id, enabled: false)
      )
      delete_payload = JSON.parse(
        Captain::Tools::Account::DeleteCaptainCustomToolService.new(assistant, user: admin).execute(tool_id: custom_tool.id)
      )
  ...
  describe 'confirmation and audit redaction' do
    ...
    it 'redacts custom tool config before confirmation preview is persisted' do
      ...
      payload = Captain::Tools::ToolConfirmationGate.new(
    ...
    it 'redacts custom tool config when confirmation preview serialization falls back' do
      ...
      gate = Captain::Tools::ToolConfirmationGate.new(
        copilot_thread: create(:captain_copilot_thread, account: account, assistant: assistant, user: admin),
        tool_definition: tool_definition,
        arguments: arguments,
        user: admin
    ...
    it 'redacts custom tool config before trace instrumentation capture', :aggregate_failures do
      service = Captain::Tools::Account::CreateCaptainCustomToolService.new(assistant, user: admin)
