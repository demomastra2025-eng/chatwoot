...
RSpec.describe 'Webhooks::InstagramController', type: :request do
  ...
  def post_instagram_webhook(payload, signature: signature_for(payload), env: { INSTAGRAM_APP_SECRET: client_secret })
  ...
  describe 'POST /webhooks/instagram' do
    ...
    it 'returns unauthorized when signature uses an unsupported prefix' do
      ...
      post_instagram_webhook(body, signature: 'sha1=invalid-signature')
    ...
    context 'when processing echo events' do
      ...
      it 'delays processing for echo events by 2 seconds after signature verification' do
