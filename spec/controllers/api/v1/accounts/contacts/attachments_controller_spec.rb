    ...
    context 'when user is logged in' do
      context 'with user as administrator' do
        it 'returns attachments from all the contact conversations' do
          get "/api/v1/accounts/#{account.id}/contacts/#{contact.id}/attachments", headers: admin.create_new_auth_token
        ...
        it 'serializes the conversation display id as conversation_id' do
          conversation = contact.conversations.first

          get "/api/v1/accounts/#{account.id}/contacts/#{contact.id}/attachments", headers: admin.create_new_auth_token

          payload = response.parsed_body['payload']
          attachment = payload.find { |item| item['conversation_id'] == conversation.display_id }
      ...
      context 'with user as agent' do
        it 'returns attachments only from inboxes the agent has access to' do
          get "/api/v1/accounts/#{account.id}/contacts/#{contact.id}/attachments", headers: agent.create_new_auth_token
