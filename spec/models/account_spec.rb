...
RSpec.describe Account do
  ...
  describe 'support_email' do
    ...
    it 'returns the support email from inbox if inbox value is present' do
      account.update(support_email: 'support@one-link.kz')
      with_modified_env MAILER_SENDER_EMAIL: 'hello@one-link.kz' do
        expect(account.support_email).to eq('support@one-link.kz')
      end
    end
