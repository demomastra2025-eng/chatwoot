...
RSpec.describe Message do
    ...
    let(:expected_data) do
      {
        ...
        conversation: {
          ...
          unread_count: message.conversation.unread_incoming_messages_count
        },
        sentiment: {},
  ...
  describe '#reopen_conversation' do
    ...
    it 'will mark the conversation as pending if a Dialogflow integration is active' do
      create(:integrations_hook, :dialogflow, inbox: conversation.inbox, account: conversation.account)
      conversation.resolved!
      message.save!
    ...
    context 'when message is input_csat' do
      ...
      context 'when inbox is not web widget' do
        ...
        it 'returns only the base content without URL when survey_url stored separately' do
          message.content_attributes = { 'survey_url' => 'https://one-link.kz/survey/responses/12345' }
          expect(message.content).to eq('Rate your experience')
        end
      end
    end
