...
RSpec.describe Telephony::LogicalCallOccurrence do
  ...
  it 'records human connected and terminal facts with immutable actor and duration snapshots' do
    ...
    team.destroy!
    session.update!(provider_call_sid: 'late-provider-sid')
    session.update!(status: 'completed', ended_at: ended_at, end_reason: 'remote_hangup')

    connected = occurrences.current_revision.find_by!(occurrence_kind: 'connected')
    expect(connected).to have_attributes(revision: 2, correlation_ref: 'late-provider-sid')
  ...
  it 'keeps answered actor and team names on later revisions after catalog renames' do
    team = create(:team, account: account, name: 'Original team')
    create(:team_member, team: team, user: user)
    session = create_session(metadata: { 'last_payload' => { 'answered_team_id' => team.id } })
    session.update!(status: 'in_progress', answered_at: started_at + 1.second, answered_by: "user:#{user.id}")
    original = occurrences.find_by!(occurrence_kind: 'connected')

    user.update!(name: 'Renamed operator')
    team.update!(name: 'Renamed team')
    session.update!(provider_call_sid: 'later-provider-sid')
    session.update!(status: 'completed', ended_at: started_at + 30.seconds)

    revised = occurrences.current_revision.find_by!(occurrence_kind: 'connected')
    terminal = occurrences.current_revision.find_by!(occurrence_kind: 'terminal')
    expect(revised).to have_attributes(revision: 2, actor_id_snapshot: user.id,
                                       actor_name_snapshot: original.actor_name_snapshot,
                                       actor_team_id_snapshot: team.id,
                                       actor_team_name_snapshot: original.actor_team_name_snapshot)
    expect(terminal).to have_attributes(actor_name_snapshot: original.actor_name_snapshot,
                                        actor_team_name_snapshot: original.actor_team_name_snapshot)
  end

  it 'keeps the answering assistant name when the catalog is renamed before a connected revision' do
    assistant = create(:captain_assistant, account: account, name: 'Original assistant')
    session = create_session(metadata: { 'captain_assistant_id' => assistant.id })
    session.update!(status: 'in_progress', answered_at: started_at + 1.second, answered_by: 'ai_agent')

    assistant.update!(name: 'Renamed assistant')
    session.update!(provider_call_sid: 'later-ai-sid')
    session.update!(status: 'completed', ended_at: started_at + 30.seconds)

    expect(occurrences.current_revision.find_by!(occurrence_kind: 'connected'))
      .to have_attributes(revision: 2, assistant_id_snapshot: assistant.id, assistant_name_snapshot: 'Original assistant')
    expect(occurrences.current_revision.find_by!(occurrence_kind: 'terminal'))
      .to have_attributes(assistant_id_snapshot: assistant.id, assistant_name_snapshot: 'Original assistant')
  end
  ...
  it 'captures AI assistant identity separately and preserves it after deletion' do
    ...
    assistant.destroy!
    session.update!(provider_call_sid: 'late-ai-sid')
    session.update!(status: 'completed', ended_at: started_at + 20.seconds)
    revised_connected = occurrences.current_revision.find_by!(source_ref: session.external_call_ref, occurrence_kind: 'connected')
    expect(revised_connected).to have_attributes(
      revision: 2, correlation_ref: 'late-ai-sid', assistant_id_snapshot: assistant.id, assistant_name_snapshot: 'Voice concierge'
    )
  ...
  it 'rejects invented historical actor references on a connected revision at the database boundary' do
    session = create_session
    session.update!(status: 'in_progress', answered_at: started_at + 1.second, answered_by: "user:#{user.id}")
    original = occurrences.find_by!(occurrence_kind: 'connected')
    foreign_actor = create(:user, account: account)
    account.account_users.find_by!(user_id: foreign_actor.id).destroy!
    invalid = original.attributes.except('id').merge(
      'revision' => 2, 'supersedes_occurrence_id' => original.id,
      'actor_id_snapshot' => foreign_actor.id
    )

    described_class.transaction(requires_new: true) do
      expect { described_class.insert_all!([invalid]) } # rubocop:disable Rails/SkipsModelValidations
        .to raise_error(ActiveRecord::StatementInvalid, /actor snapshot must belong to account/)
      raise ActiveRecord::Rollback
    end
  end
