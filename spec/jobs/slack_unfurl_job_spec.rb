...
RSpec.describe SlackUnfurlJob do
  ...
  let(:link_shared) do
    {
      ...
      event: {
        ...
        channel: 'G01354F6A6Q',
        links: [{
          url: "https://one-link.kz/app/accounts/#{hook.account_id}/conversations/#{conversation.display_id}",
          domain: 'one-link.kz'
        }]
      },
      type: 'event_callback',
  ...
  context 'when the calls the slack unfurl job' do
    ...
    context 'when another account URL is shared' do
      before do
        link_shared[:event][:links][0][:url] = 'https://one-link.kz/app/accounts/123/conversations/123'
      end
