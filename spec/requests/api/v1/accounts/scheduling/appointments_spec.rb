...
RSpec.describe 'Scheduling Appointments API', type: :request do
  ...
  it 'paginates the policy-scoped appointment list with stable ordering and count' do
    appointments = Array.new(51) do
      create(
    ...
    create(
      ...
      resource: create(:scheduling_resource, account: other_account),
      starts_at: booking_day,
      ends_at: booking_day + 30.minutes
    )
    ...
    get "/api/v1/accounts/#{account.id}/scheduling/calendar",
        params: {
          ...
          to: (booking_day + 2.days).beginning_of_day.iso8601,
    ...
    expect(response_body.fetch('meta')).to include('count' => 51, 'page' => 2, 'per_page' => 25)
    expect(response_body.dig('payload', 'appointments').pluck('id')).to eq(appointments[25, 25].map(&:id))
  end

  it 'falls back to bounded pagination defaults for structured page parameters' do
    get "/api/v1/accounts/#{account.id}/scheduling/calendar",
        params: {
          view: 'list',
          from: booking_day.beginning_of_day.iso8601,
    ...
    expect(response).to have_http_status(:ok)
    expect(response_body.fetch('meta')).to include(
      'page' => 1,
      'per_page' => Api::V1::Accounts::Scheduling::CalendarController::DEFAULT_APPOINTMENTS_PER_PAGE
    )
  end
  ...
  it 'derives catalog pricing without a manual amount override' do
  ...
  it 'rejects direct cancellation of imported appointments' do
    appointment = create(
      :scheduling_appointment,
      resource: resource,
      account: account,
  ...
  it 'returns a stable calendar payload shape' do
    ...
    expect(response_body['payload'].keys).to include(
      ...
      'time_offs', 'appointments', 'slots'
    )
    expect(response_body['payload'].keys).not_to include('payments', 'expenses')
    expect(response_body.dig('payload', 'appointments', 0, 'conversation_display_id')).to eq(conversation.display_id)
    expect(response_body.dig('payload', 'appointments', 0, 'service_amount')).to eq(20_000)
  end
  ...
  it 'filters calendar appointments by status' do
  ...
  it 'returns saved service amounts without exposing historical finance details' do
    appointment = create(
      :scheduling_appointment,
      resource: resource,
      account: account,
    ...
    expect(response_body.fetch('payload').keys).not_to include(
      'settlement_amount', 'payment_status', 'payments', 'expense'
    )
    ...
    expect(response).to have_http_status(:ok)
    expect(response_body.dig('payload', 'appointments', 0, 'service_amount')).to eq(20_000)
    expect(response_body.dig('payload', 'appointments', 0).keys).not_to include('payment_status')
    expect(response_body.fetch('payload').keys).not_to include('payments', 'expenses')
  end

  it 'does not gate saved service amounts behind a retired finance capability' do
    contact.update!(owner: agent)
    ...
    expect(payloads.fetch(other_appointment.id)).to include('service_amount' => 30_000)

    get "#{path}/#{other_appointment.id}", headers: headers, as: :json

    expect(response_body.dig('payload', 'service_amount')).to eq(30_000)
    ...
    expect(calendar_appointments.fetch(other_appointment.id)).to include('service_amount' => 30_000)
  end
