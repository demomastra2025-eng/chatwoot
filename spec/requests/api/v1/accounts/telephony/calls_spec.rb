...
RSpec.describe 'Telephony Calls API', type: :request do
  ...
  it 'returns and streams an external Sipuni recording through the OneLink recording route' do
    ...
    external_url = 'https://sipuni.com/api/crm/record?id=sipuni-call-1&hash=recording-hash&user=056124'
    call_session = create(
      :telephony_call_session,
      account: account,
      ...
      provider: 'sipuni',
      external_call_ref: 'sipuni-recording-call',
      recording_ref: external_url,
      metadata: {
        'recording' => {
          'recording_ref' => external_url,
    ...
    stub_request(:get, external_url)
      .to_return(status: 200, body: "ID3\x04sipuni-audio", headers: { 'Content-Type' => 'audio/mpeg' })
