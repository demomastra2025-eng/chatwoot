...
RSpec.describe AutomationRulePolicy, type: :policy do
  ...
  let(:actions) { %i[index? show? create? update? clone? destroy? resume?] }
  ...
  it 'accepts a matching explicit grant while shadow mode preserves legacy authority' do
    custom_role = create(:custom_role, account: account, permissions: %w[automation_manage])
    account_user.update!(custom_role: custom_role)
    stub_mode_resolution(mode: 'shadow', scope: 'all')
