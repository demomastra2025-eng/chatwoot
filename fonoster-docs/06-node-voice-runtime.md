...
The runtime can also consume `app` or `ai` decisions only when the bridge
returns a concrete dialable destination. Direct live handoff to another
Fonoster application by `appRef` is not implemented in this runtime yet.
