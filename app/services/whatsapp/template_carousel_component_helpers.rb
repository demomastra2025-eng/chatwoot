module Whatsapp::TemplateCarouselComponentHelpers
  ...
  def process_carousel_component(template, processed_params)
    ...
    cards = template_cards.each_with_index.map do |template_card, card_index|
...

      {
        ...
        components: build_carousel_card_components(template_card, submitted_card, card_index)
      }
    end
  ...
  def build_carousel_card_components(template_card, submitted_card, card_index)
    components = [build_carousel_header(template_card, submitted_card, card_index)]
  ...
  def build_carousel_header(template_card, submitted_card, card_index)
    ...
    media_id = submitted_card.dig('header', 'media_id').to_s.strip
    raise ArgumentError, "Carousel card #{card_index + 1} Meta media ID is required" if media_id.blank?
