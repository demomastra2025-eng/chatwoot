...
class Telephony::LogicalCallOccurrenceRecorder # rubocop:disable Metrics/ClassLength
  ...
  def initialize(call_session:, allow_attempted: false)
  ...
  def record!
    ...
    with_account_snapshot_lock { with_logical_call_lock { record_under_lock! } }
  end
  ...
  def record_under_lock!
    @recorded_at = Time.current.utc.ceil(6)
  ...
  def with_account_snapshot_lock
    # Hold the shared barrier through the outer CallSession commit, not just this callback.
    ActiveRecord::Base.connection.execute(
      "SELECT pg_advisory_xact_lock_shared(#{Telephony::LogicalCallOccurrence.account_snapshot_lock_key(call_session.account_id)})"
    )
    yield
  end
  ...
  def record_revisable_occurrence!(kind, attributes)
    ...
    # Lock ownership serializes siblings; keep each successor later even when the clock has not advanced.
    revision_time = [Time.current, attributes.fetch(:created_at), latest&.created_at&.+(Rational(1, 1_000_000))].compact.max
    Telephony::LogicalCallOccurrence.create!(
      attributes.merge(
        created_at: revision_time,
        reliable_since: revision_time,
  ...
  def occurrence_attributes(kind, source, sessions) # rubocop:disable Metrics/AbcSize, Metrics/CyclomaticComplexity, Metrics/MethodLength
...

    {
      ...
      **actor,
      ...
      reliability: occurrence_reliability(kind, connected_at, duration),
  ...
  def occurrence_reliability(kind, connected_at, duration)
    return 'unknown' if kind == 'connected' && connected_at.blank?
    return 'unknown' if kind == 'terminal' && duration.nil?
  ...
  def occurrence_actor_snapshot(kind, connected_source)
    current = actor_snapshot(connected_source)
    return current if kind == 'attempted'

    connected_occurrence = Telephony::LogicalCallOccurrence.current_revision.find_by(occurrence_identity('connected'))
    return current if connected_occurrence.blank?

    historical = connected_occurrence.attributes.symbolize_keys.slice(
    ...
    return historical if kind == 'terminal'

    preserved_connected_actor(connected_source, connected_occurrence, current, historical)
  end

  def preserved_connected_actor(source, connected_occurrence, current, historical)
    return current unless same_answer_actor?(source, connected_occurrence)

    team_id = answered_team_id(source)
    if team_id.present? && team_id.to_i != connected_occurrence.actor_team_id_snapshot
      historical.except!(:actor_team_id_snapshot, :actor_team_name_snapshot)
    end
    # A catalog rename is not a new answer: keep the name captured when the actor answered.
    # Only an explicit change of answered team may replace the prior team attribution.
    historical.merge(current) { |_key, previous, fresh| previous || fresh }
  end

  def same_answer_actor?(source, connected_occurrence)
    kind, actor_id = actor_identity(source)
    return false unless kind == connected_occurrence.actor_kind

    case kind
    when 'human' then actor_id == connected_occurrence.actor_id_snapshot
    when 'ai_agent' then assistant_id(source).to_i == connected_occurrence.assistant_id_snapshot.to_i
    else false
    end
  end
  ...
  def answered_team_id(source)
    metadata_sources(source).filter_map { |metadata| metadata['answered_team_id'].presence || metadata['answeredTeamId'].presence }.first
  end

  def explicit_assistant(source)
    id = assistant_id(source)
    return if id.blank?

    Captain::Assistant.find_by(account_id: call_session.account_id, id: id)
  end

  def assistant_id(source)
    metadata_sources(source).filter_map do |metadata|
