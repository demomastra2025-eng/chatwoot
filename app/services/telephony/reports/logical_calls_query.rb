...
class Telephony::Reports::LogicalCallsQuery # rubocop:disable Metrics/ClassLength
  ...
  def initialize(account:, occurrences_scope:, params: {})
    ...
    normalize_window!
    normalize_as_of!
    # Once this exclusive barrier is acquired, every earlier writer has committed.
    # Later writers stamp their revisions after the barrier, strictly beyond this fixed as_of.
    Telephony::LogicalCallOccurrence.transaction do
      Telephony::LogicalCallOccurrence.connection.execute(
        "SELECT pg_advisory_xact_lock(#{Telephony::LogicalCallOccurrence.account_snapshot_lock_key(account.id)})"
      )
      if occurrences_scope.is_a?(Telephony::LogicalCallOccurrencePolicy::Scope)
        @occurrences_scope = occurrences_scope.resolve(as_of: as_of)
        @effective_inbox_sql = occurrences_scope.effective_inbox_sql(as_of: as_of)
      else
        @occurrences_scope = occurrences_scope
        @effective_inbox_sql = 'telephony_logical_call_occurrences.inbox_id_snapshot'
      end
      normalize_query!
    end
  end
  ...
  attr_reader :dimension, :effective_inbox_sql, :filters, :long_threshold_seconds, :metric, :occurrences_scope
  ...
  def validate_filter_ids!
    ...
    validate_inbox_id_filter!(relation)
    {
      actor_id: :actor_id_snapshot,
      actor_team_id: :actor_team_id_snapshot,
      assistant_id: :assistant_id_snapshot
  ...
  def validate_inbox_id_filter!(relation)
    return unless filters[:inbox_id]

    raise_validation!('inbox_id is invalid') unless relation.exists?(["#{effective_inbox_sql} = ?", filters[:inbox_id]])
  end
  ...
  def filtered_occurrences
    @filtered_occurrences ||= filters.reduce(windowed_current_occurrences) do |relation, (key, value)|
      key == :inbox_id ? relation.where("#{effective_inbox_sql} = ?", value) : relation.where(FILTER_COLUMNS.fetch(key) => value)
    end
  end
  ...
  def aggregate_metric(metric_name)
    ...
    group_columns = dimension == 'inbox' ? [Arel.sql(effective_inbox_sql)] : columns
    rows = grouped_reliability_counts(metric_name, group_columns).map do |values, reliability_counts|
  ...
  def details_result
    @details_result ||= begin
      ...
      rows = relation.select('telephony_logical_call_occurrences.*', "#{effective_inbox_sql} AS effective_inbox_id")
                     .order(occurred_at: :desc, id: :desc).limit(per_page).offset(page_offset)
  ...
  def detail_payload(occurrence, metric_name)
    {
      ...
      effective_inbox_id: occurrence.read_attribute(:effective_inbox_id),
  ...
  def effective_inbox_ids_digest
    ids = filtered_occurrences.distinct.pluck(Arel.sql(effective_inbox_sql)).compact.sort
