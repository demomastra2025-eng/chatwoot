class Crm::Tasks::CustomFieldSearchQuery
  SQL = <<~SQL.squish.freeze
    EXISTS (
      SELECT 1
      FROM jsonb_each(crm_tasks.custom_attributes) AS custom_value(key, value)
      INNER JOIN crm_field_definitions
        ON crm_field_definitions.account_id = crm_tasks.account_id
        AND crm_field_definitions.entity_kind = 'task'
