class Crm::Deals::AssignmentAuthorizer
  ...
  def deny_assignment!
    raise Crm::Error.new(
      ...
      message: 'Deal owner or team is outside the assignment scope',
      status: :forbidden,
  ...
  def assignment_allowed?(access_scope)
    ...
    when 'own' then owner == actor && (team.blank? || actor_team_ids.include?(team.id))
    else false
    end
  end

  def team_assignment_allowed?
    return owner == actor if team.blank?
    return false unless actor_team_ids.include?(team.id)

    owner.blank? || TeamMember.exists?(team_id: team.id, user_id: owner.id)
  end
