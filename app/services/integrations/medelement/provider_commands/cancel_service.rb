class Integrations::Medelement::ProviderCommands::CancelService
  ...
  def cancelled_attributes
    cancellation_type = command.reconciliation_required? ? 'reconciliation' : 'user_action'
    {
      status: 'cancelled',
      executed_at: now,
      last_error_code: "#{cancellation_type}_cancelled_manually",
      last_error_status: nil,
      execution_state: command.execution_state.to_h
                              .except('reconciliation_next_at')
                              .merge(
                                "#{cancellation_type}_cancelled_at" => now.iso8601,
                                "#{cancellation_type}_cancelled_by_id" => actor.id
                              )
    }
  end
end
