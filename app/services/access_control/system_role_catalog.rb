class AccessControl::SystemRoleCatalog
  ...
  # automation_rules is advertised for policy resolution before its database constraint ships.
  BOOTSTRAP_RESOURCES = (AccessRoleGrant::RESOURCES - %w[automation_rules]).freeze
  ...
  class << self
    ...
    def observer_grants
      grants_for_resources(BOOTSTRAP_RESOURCES, only: %w[view], scope: 'all')
    end
