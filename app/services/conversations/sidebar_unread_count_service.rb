class Conversations::SidebarUnreadCountService
  ...
  def perform
...

    {
      ...
      statuses: normalize_enum_counts(unread_scope.group(:status).count('messages.id'), Conversation.statuses),
      inboxes: normalize_counts(unread_scope.group(:inbox_id).count('messages.id')),
      teams: normalize_counts(unread_scope.where.not(team_id: nil).group(:team_id).count('messages.id')),
