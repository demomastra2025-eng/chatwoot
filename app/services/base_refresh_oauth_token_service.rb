class BaseRefreshOauthTokenService
  MissingRefreshTokenError = Class.new(StandardError)
  ...
  def update_channel_provider_config(new_tokens)
    channel.provider_config = provider_config.merge(
  ...
  def build_token_service(oauth_strategy)
    raise MissingRefreshTokenError, 'A refresh_token is not available' if provider_config[:refresh_token].blank?
