class Notification::PushTestService
  ...
  def test_fcm_direct(subscription)
    ...
    result(subscription, 'fcm', status, formatted_response_message(status_code, response[:body], subscription))
  rescue StandardError => e
  ...
  def test_fcm_via_hub(subscription)
    ...
    result(subscription, 'fcm_via_hub', :success, formatted_response_message(response.code, response.body, subscription))
  rescue RestClient::ExceptionWithResponse => e
    result(subscription, 'fcm_via_hub', :failure, formatted_response_message(e.response&.code, e.response&.body, subscription))
  rescue StandardError => e
  ...
  def formatted_response_message(status_code, body, subscription)
    "HTTP #{status_code} — #{sanitized_response_body(body, subscription)}"
  end

  def sanitized_response_body(body, subscription)
    ...
    sensitive_values.reduce(body.to_s) do |message, value|
      message.gsub(value.to_s, redacted_value(value))
    end
  end
