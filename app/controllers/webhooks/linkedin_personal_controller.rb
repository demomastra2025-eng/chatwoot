class Webhooks::LinkedinPersonalController < ActionController::API
  ...
  def process_payload
    ...
    Rails.logger.error("[LINKEDIN PERSONAL] Webhook processing failed: #{e.message}")
  ...
  def set_channel
    @channel = Channel::LinkedinPersonal.find_by!(webhook_identifier: params[:webhook_identifier])
  end
  ...
  def webhook_data
    ...
    params.to_unsafe_hash
          .except('controller', 'action', 'webhook_identifier', 'event', 'linkedin_personal')
          .deep_symbolize_keys
  end

  def job_class
    return Channels::LinkedinPersonal::ProcessHistoryWebhookEventJob if HISTORY_EVENTS.include?(params[:event].to_s)

    Channels::LinkedinPersonal::ProcessWebhookEventJob
  end
