...
class Meta::InstagramCredentialHealthCheckJob < ApplicationJob
  ...
  def active_channels(after_id)
    scope = Channel::Instagram.joins(:account)
                              .where(accounts: { status: Account.statuses[:active] })
                              .order(:id)
    after_id ? scope.where('channel_instagram.id > ?', after_id) : scope
  end
