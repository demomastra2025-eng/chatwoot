...
#  active_web_desktop_auth_client_set_at :datetime
#  active_web_mobile_auth_client_set_at  :datetime
...
#  active_web_desktop_auth_client_id     :string
#  active_web_mobile_auth_client_id      :string
#
# Indexes
#
...
class User < ApplicationRecord
  ...
  # DeviseTokenAuth checks devise_modules while its concern is included above,
  # before this model declares :confirmable. Include its reconfirmation support
  # now so Devise 5 can detect email changes despite DTA's token-auth override.
  ...
  has_one :telephony_agent_binding, dependent: :destroy, class_name: 'Telephony::AgentBinding'
  ...
  # Lifecycle facts must be written while account memberships still exist.
