...
class Telephony::LogicalCallOccurrence < ApplicationRecord
  ...
  # A report's fixed as_of must not see a revision stamped inside a transaction that commits later.
  # Writers share this account lock (different calls can proceed); readers take its exclusive counterpart.
  def self.account_snapshot_lock_key(account_id)
    Digest::SHA256.digest("telephony-logical-occurrences:account:#{account_id}").unpack1('q>')
  end
  ...
  def historical_connected_snapshot?(attribute, value)
    return false unless %w[connected terminal].include?(occurrence_kind)

    prior = self.class.where(
      account_id: account_id,
      logical_call_identity: logical_call_identity,
      occurrence_kind: 'connected',
      source_version: source_version
    ).where(attribute => value)
    prior = prior.where(id: supersedes_occurrence_id) if occurrence_kind == 'connected'
    prior.exists?
  end
