...
export function useBranding() {
  ...
  /**
   * Replaces legacy/default brand names in text with the installation name from global config
   * @param {string} text - The text to process
   * @returns {string} - Text with the default brand token replaced by installation name
   */
  const replaceInstallationName = text => {
    ...
    return text.replace(/(?:OneLink|Chatwoot)/g, installationName);
  };
