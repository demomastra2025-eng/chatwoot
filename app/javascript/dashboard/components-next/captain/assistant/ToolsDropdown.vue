...
const overlayDialogRef = ref(null);
...
const closeOverlay = () => {
  if (overlayDialogRef.value?.open) {
    overlayDialogRef.value.close();
  }
...
watch(
  ...
  async isOverlay => {
    ...
    await nextTick();
    if (overlayDialogRef.value && !overlayDialogRef.value.open) {
      overlayDialogRef.value.showModal();
    }
  },
  { immediate: true }
);
...
<template>
  <TeleportWithDirection v-if="overlay" to="body">
    <dialog
      ...
      class="tools-dropdown-dialog m-auto h-full max-h-none w-full max-w-none border-0 bg-transparent p-0"
      @cancel.prevent="closeOverlay"
    >
      <div
        class="flex min-h-full items-center justify-center p-4"
        data-test-id="tools-dropdown-overlay"
...
<style scoped>
.tools-dropdown-dialog::backdrop {
