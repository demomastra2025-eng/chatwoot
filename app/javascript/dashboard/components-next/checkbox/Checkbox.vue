...
<template>
  <div class="relative w-4 h-4">
    <input
      ...
      type="checkbox"
      :disabled="disabled"
      class="peer absolute inset-0 z-10 h-4 w-4 disabled:opacity-50 appearance-none rounded border border-n-slate-6 ring-transparent transition-all duration-200 checked:border-n-brand-solid checked:bg-n-brand-solid dark:border-gray-600 dark:checked:border-n-brand-solid indeterminate:border-n-brand-solid indeterminate:bg-n-brand-solid hover:enabled:bg-n-blue-border cursor-pointer"
      @change="handleChange"
    />
    <!-- Checkmark SVG -->
    <svg
