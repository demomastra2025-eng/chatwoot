...
.vuecal__event(
  ...
  .vuecal__event-resizer(v-if="isResizable" @click.stop.prevent @dragstart.prevent.stop)
...
const classes = computed(() => {
  ...
  const isCutStart =
    !props.inAllDayBar &&
    (event._?.startMinutes < config.timeFrom ||
      (isMultiday && !eventStartsInThisCell.value));
  const isCutEnd =
    !props.inAllDayBar &&
    (event._?.endMinutes > config.timeTo ||
      (isMultiday && !eventEndsInThisCell.value));
  ...
  return {
    ...
    'vuecal__event--all-day':
      event.allDay ||
      (event._?.startMinutes === 0 && event._?.duration === 24 * 60),
...
const eventStartsInThisCell = computed(() => {
  if (event._.multiday) {
    return (
      new Date(event.start).setHours(0, 0, 0, 0) === props.cellStart.getTime()
    );
  }
  return true;
});
...
const eventEndsInThisCell = computed(() => {
  if (event._.multiday) {
    return dateUtils.isSameDate(
      new Date(new Date(event.end).setMilliseconds(-1)),
      props.cellEnd
    );
  }
  return true;
...
const styles = computed(() => {
  const hasPosition =
    (view.isDay || view.isDays || view.isWeek) &&
    config.time &&
    !props.inAllDayBar;
  const isHzl = config.horizontal;
  ...
  if (hasPosition) {
    ...
    const to =
      Math.min(config.timeTo, endMinutes) +
      (event._.duration && !endMinutes ? 24 * 60 : 0);
...
const eventListeners = computed(() => {
  ...
  eventListeners.click = e => {
    if (
      suppressNextClick ||
      touch.fromResizer ||
      globalTouchState.isResizingEvent
    ) {
...
const onMousedown = e => {
  ...
  const currentEventEl = eventEl.value;
  ...
  if (!cachedRect || now - rectCacheTime > RECT_CACHE_DURATION) {
    cachedRect = currentEventEl.getBoundingClientRect();
    rectCacheTime = now;
  }
  ...
  touch.cellEl = currentEventEl.closest('.vuecal__cell');
  // Store the event start to apply on event end when resizing and end < start.
  touch.resizeStartDate = event.start;

  if (touch.fromResizer) handleEventResize(e, event, currentEventEl);
