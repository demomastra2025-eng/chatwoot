...
export const useEvents = vuecal => {
  ...
  const events = computed(() => {
    ...
    const sortedEvents = config.events
      .slice()
      .sort((a, b) => (a.start - b.start < 0 ? -1 : 1));
    ...
    for (let event of sortedEvents) {
      ...
      const missingMethods =
        !event._?.register || !event.isOverlapping || !event.delete;
      ...
      // Remove 1ms in case the event ends at next midnight 00:00:00.
        ...
        !dateUtils.isSameDate(event.start, new Date(event.end.getTime() - 1))
      ) {
        event._.multiday = config.multidayEvents;
        if (!config.multidayEvents) {
          console.info(
            'Vue Cal: Multi-day events provided without being enabled. Truncating event end to next midnight.'
          );
        ...
        if (!events.byDate[event._.startFormatted])
          events.byDate[event._.startFormatted] = [];
        ...
        if (!events.byYear[year][month][day])
          events.byYear[year][month][day] = [];
  ...
  const normalizeEventDates = event => {
    ...
    if (typeof event.start === 'string')
      event.start = dateUtils.stringToDate(event.start);
    if (typeof event.end === 'string')
      event.end = dateUtils.stringToDate(event.end);
    ...
    if (
      ...
      if (isNaN(event.start))
        console.error(
          `Vue Cal: invalid start date for event "${event.title}".`,
          event.start
        );
      else if (isNaN(event.end))
        console.error(
          `Vue Cal: invalid end date for event "${event.title}".`,
          event.end
        );
      else
        console.error(
          `Vue Cal: invalid event dates for event "${event.title}". The event ends before it starts.`,
          event.start,
          event.end
        );
      return false;
  ...
  const injectMetaData = event => {
    ...
    event._.multiday = !dateUtils.isSameDate(
      event.start,
      new Date(event.end.getTime() - 1)
    ); // Remove 1ms if end is equal to next midnight.
  ...
  const createEvent = newEvent => {
    if (!newEvent.start || !newEvent.end) {
      console.error(
        'Vue Cal: Cannot create an event without valid start and end dates.'
      );
      return;
    }
  ...
  const deleteEvent = async (eventIdOrCriteria, forcedStage = 0) => {
    if (!eventIdOrCriteria)
      return console.warn(
        'Vue Cal: Cannot delete event without its ID or criteria.'
      );
    let eventId =
      typeof eventIdOrCriteria === 'string' || !isNaN(eventIdOrCriteria)
        ? eventIdOrCriteria
        : null;
    const eventCriteria =
      typeof eventIdOrCriteria === 'object'
        ? Object.entries(eventIdOrCriteria)
        : null;
    if (eventCriteria) {
      const [criteriaKey, criteriaValue] = eventCriteria[0];
      eventId = config.events.find(
        event => event[criteriaKey] === criteriaValue
      )?._.id;
    }

    if (!config.editableEvents.delete) {
      return console.info(
        'Vue Cal: Event deletion is disabled. Enable it with the `editable-events` props.'
      );
    }
    if (!eventId)
      return console.warn('Vue Cal: Cannot delete event without its ID.');

    const index = config.events.findIndex(item => item._.id === eventId);
    if (index === -1)
      return console.warn(
        `Vue Cal: Cannot delete unknown event \`${eventId}\`.`
      );

    const event = config.events[index];
    if (event.deletable === false)
      return console.warn(
        `Vue Cal: Can't delete event \`${eventId}\` since it was explicitely set to \`delete: false\`.`
      );
    ...
    switch (forcedStage) {
      ...
      case 2:
        ...
        event._.$el?.dispatchEvent(
          new CustomEvent('event-deleted', { detail: event._.id })
        );
        break;
  ...
  const getCellOverlappingEvents = (cellStart, cellEnd, allDay) => {
    ...
    for (const e of cellEvents) {
      ...
      const currentOverlaps = activeEvents.filter(active => {
        const sameSchedule =
          !config.schedules?.length || e.schedule === active.schedule;
      ...
      cellOverlaps[id].maxConcurrent = Math.max(
        currentOverlaps.length + 1,
        inheritedMax
      );
      ...
      for (const activeEvent of currentOverlaps) {
        ...
        cellOverlaps[activeEvent._.id].maxConcurrent =
          cellOverlaps[id].maxConcurrent;
      }
  ...
  const getEventsInRange = (
    ...
    { excludeIds = [], schedule = null, background = true, allDay = false } = {}
  ) => {
    ...
    if (totalEvents <= 100) {
      for (const event of Object.values(byId)) {
        ...
        if (
          event.start.getTime() < rangeEndTimestamp &&
          event.end.getTime() > rangeStartTimestamp
        )
      ...
      for (let month = monthFrom; month <= monthTo; month++) {
        ...
        for (const dayStr in days) {
          ...
          for (let i = 0; i < dayEventIds.length; i++) {
            ...
            // Accept events that overlap the range.
            if (
              e.start.getTime() < rangeEndTimestamp &&
              e.end.getTime() > rangeStartTimestamp
            )
              eventsArray.push(e);
          }
        }
  ...
  const isEventInRange = (event, start, end) => {
    ...
    const endTimestamp = allDayOrTimeless
      ...
      : event.end.getTime();
    const rangeStart = allDayOrTimeless
      ? new Date(start).setHours(0, 0, 0, 0)
      : start.getTime();
    const rangeEnd = allDayOrTimeless
  ...
  const computeEventStartEnd = (event, cellStart) => {
    ...
    const movePercentage =
      resizeState[config.horizontal ? 'movePercentageX' : 'movePercentageY'];
  ...
  const onDocumentMousemove = async e => {
    ...
    if (resizeState.cellEl) {
      const { top, left, width, height } =
        resizeState.cellEl.getBoundingClientRect();
    ...
    if (resizeState.fromResizer && resizeState.resizingEvent) {
      ...
      const { newStart, newEnd } = computeEventStartEnd(
        resizeState.resizingEvent,
        cellStart
      );
      ...
      if (resizeEventHandler) {
        acceptResize = await resizeEventHandler({
          ...
          overlaps: getEventOverlaps(resizeState.resizingEvent, {
            start: newStart,
            end: newEnd,
          }),
        });
      ...
      if (acceptResize !== false) {
        ...
        if (resizeState.resizingLastAcceptedEvent)
  ...
  const onDocumentMouseup = async e => {
    if (vuecal.touch?.isResizingEvent && resizeState.resizingEvent) {
      ...
      if (resizeState.resizingEvent._.duration < 1) {
        resizeState.resizingEvent.start =
          resizeState.resizingOriginalEvent.start;
  ...
  const handleEventResize = (e, event, eventEl) => {
    ...
    resizeState.fromResizer = !!domEvent.target.closest(
