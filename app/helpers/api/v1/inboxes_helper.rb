module Api::V1::InboxesHelper
  def inbox_name(channel)
    ...
    return channel.generated_inbox_name if channel.is_a?(Channel::Weixin)
  ...
  def open_imap_connection(channel_data, authentication)
    imap_enable_ssl = channel_data.key?(:imap_enable_ssl) ? channel_data[:imap_enable_ssl] : true
    imap = Net::IMAP.new(channel_data[:imap_address], port: channel_data[:imap_port].to_i, ssl: imap_enable_ssl)
