module Instagram::IntegrationHelper
  ...
  # @param inbox_id [Integer, nil] Optional inbox ID for reauthorization flows
  # @return [String, nil] The encoded JWT token or nil if client secret is missing
  def generate_instagram_token(account_id, inbox_id: nil)
    return if client_secret.blank?

    JWT.encode(token_payload(account_id, inbox_id: inbox_id), client_secret, 'HS256')
  rescue StandardError => e
    Rails.logger.error("Failed to generate Instagram token: #{e.message}")
    nil
  end

  def token_payload(account_id, inbox_id: nil)
    {
      sub: account_id,
      inbox_id: inbox_id,
  ...
  def verify_instagram_token(token)
    instagram_token_payload(token)&.[]('sub')
  end
