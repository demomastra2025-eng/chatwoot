module Enterprise::Message
  ...
  def captain_human_control_candidate?
    human_response? &&
      ...
      captain_enabled_on_thread?
  end

  def captain_enabled_on_thread?
    thread = conversation.communication_thread
    return ::CaptainInbox.exists?(inbox_id: conversation.inbox_id) unless thread

    ::CaptainInbox.where(inbox_id: thread.conversations.select(:inbox_id)).exists?
  end
