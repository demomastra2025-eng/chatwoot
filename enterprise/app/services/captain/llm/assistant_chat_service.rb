class Captain::Llm::AssistantChatService < Llm::BaseAiService
  ...
  def build_tools
    ...
    tools + @assistant.account.captain_custom_tools.enabled.map do |custom_tool|
  ...
  def custom_tools_metadata
    @assistant.account.captain_custom_tools.enabled.map do |custom_tool|
      { name: custom_tool.slug, description: custom_tool.description }
    end
  end
