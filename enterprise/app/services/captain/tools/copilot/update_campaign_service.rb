...
class Captain::Tools::Copilot::UpdateCampaignService < Captain::Tools::Copilot::CampaignAdminTool
  ...
  def build_attrs(kwargs)
    ...
    attrs[:audience] = parse_json_array(kwargs[:audience_json], field_name: 'audience_json') if kwargs.key?(:audience_json) && !kwargs[:audience_json].nil?
    attrs[:scheduled_at] = parse_campaign_datetime(kwargs[:scheduled_at], field_name: 'scheduled_at') if kwargs.key?(:scheduled_at)
    attrs[:template_params] = parse_json_hash(kwargs[:template_params_json], field_name: 'template_params_json', default: nil) if kwargs.key?(:template_params_json)
    attrs[:trigger_rules] = parse_json_hash(kwargs[:trigger_rules_json], field_name: 'trigger_rules_json', default: {}) if kwargs.key?(:trigger_rules_json)
