class Captain::Tools::Copilot::SearchDealsService < Captain::Tools::Copilot::BaseAccountTool
  ...
  def execute(query: nil, stage_name: nil, owner_id: nil, company_id: nil, archived: nil)
    deals = account.crm_deals.includes(:pipeline, :stage, :owner, :team, :company, :contacts)
    ...
    if query.present?
      deals = deals.where('crm_deals.title ILIKE :query OR crm_deals.external_ref ILIKE :query', query: "%#{query.strip}%")
    end

    formatted_collection(deals.ordered.limit(MAX_RESULTS))
  end
