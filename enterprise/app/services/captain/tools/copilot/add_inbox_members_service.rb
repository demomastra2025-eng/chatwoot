...
class Captain::Tools::Copilot::AddInboxMembersService < Captain::Tools::Copilot::BaseAccountTool
  def self.name
    'add_inbox_members'
  end
  ...
  def execute(inbox_id:, user_ids:)
    ...
    inbox.add_members(ids_to_add) if ids_to_add.any?

    formatted_payload(
      action: 'add_inbox_members',
      inbox: inbox_payload(inbox),
      added_user_ids: ids_to_add,
