...
# Keep existing indexes stable because enum values are stored in the database.
...
LANGUAGES_CONFIG = {
  ...
  41 => { name: 'қазақ тілі (kk)', iso_639_3_code: 'kaz', iso_639_1_code: 'kk', enabled: true }
}.freeze

ENABLED_LANGUAGES_CONFIG = LANGUAGES_CONFIG.select { |_key, val| val[:enabled] }.freeze

Rails.configuration.i18n.available_locales = ENABLED_LANGUAGES_CONFIG.map { |_index, lang| lang[:iso_639_1_code].to_sym }
Rails.configuration.i18n.default_locale = :ru
