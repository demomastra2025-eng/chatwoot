class ExpandMedelementUnfinishedCommandIndexes < ActiveRecord::Migration[7.1]
  OLD_UNFINISHED_STATUS_PREDICATE =
    "status IN ('awaiting_confirmation', 'queued', 'processing', 'reconciliation_required')".freeze
  UNFINISHED_STATUS_PREDICATE = <<~SQL.squish.freeze
    status IN (
  ...
  def down
    replace_indexes!(OLD_UNFINISHED_STATUS_PREDICATE)
  end
  ...
  def assert_no_expanded_duplicates!
    ...
    raise ActiveRecord::MigrationError,
          "Cannot expand Medelement unfinished-command indexes: #{duplicates} duplicate target group(s) require review"
  end
