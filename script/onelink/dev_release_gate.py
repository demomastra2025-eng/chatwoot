#!/usr/bin/env python3
"""Fail-closed ancestry and change-plan gate for OneLink DEV releases."""
...
def critical_revert_commits(repo: Path, live_sha: str, candidate_sha: str) -> list[str]:
    output = git(
        repo,
        "log",
        "--format=%H%x09%s",
        f"{live_sha}..{candidate_sha}",
        "--",
        *CRITICAL_PREFIXES,
    ).stdout
...
def evaluate(repo: Path, live_sha: str, candidate_sha: str, allow_rollback: bool = False) -> list[Change]:
    ...
    for change in changes:
        marker = " critical" if change.critical else ""
        joined_paths = "\t".join(change.paths)
        print(f"change={change.status}\t{joined_paths}{marker}")

    critical_deletions = [change for change in changes if change.critical and change.deleted]
    reverts = critical_revert_commits(repo, live_sha, candidate_sha) if descendant else []
    for revert in reverts:
        print(f"critical_revert={revert}")

    if not descendant and not allow_rollback:
        raise GateError("candidate is divergent/non-descendant from the actual live SHA")
    if (critical_deletions or reverts) and not allow_rollback:
        raise GateError("candidate removes or explicitly reverts scheduling/provider-critical code")

    print("gate_status=rollback-authorized" if allow_rollback and not descendant else "gate_status=pass")
    return changes
...
def main() -> int:
    ...
    try:
        ...
        evaluate(args.repo, live_sha, args.candidate, allow_rollback=args.allow_rollback)
